...
def generate_number_choice(correct_answer):
    ...
    for i in range(4):
        ...
        while attempts < 100:
            ...
            if (distractor not in options and 
                distractor != correct_answer and 
                all(abs(distractor - opt) >= min_gap for opt in options if opt is not None)):
...
def generate_interval_choices(correct_answer):
    ...
    intervals = [
        (split_points[i], split_points[i+1]) for i in range(4)
    ]

    correct_interval_index = next(i for i, interval in enumerate(intervals) if interval[0] < correct_answer < interval[1])
...
def generate_trend_quant(numbers):
    if len(numbers) < 2:
        return ['Stable', 'Fluctuating, overall stable', 'Steady increase by 1.0%', 'Steady decrease by 1.0%'], "Stable"
...
if __name__ == "__main__":
    print(generate_interval_choices(0.5))
    print(generate_interval_choices(0))
    print(generate_number_choice(0.23))