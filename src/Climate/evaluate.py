...
for model in models:
    ...
    pis = 0
    for d in data:
        pn_correct = f"the answer is {d['Correct']}".lower() in d[model + "pn"].lower()
        pi_correct = json.loads(d[model][-1]["content"])["parameters"]["answer"].lower() == d["Correct"].lower() if d[model] else 0
