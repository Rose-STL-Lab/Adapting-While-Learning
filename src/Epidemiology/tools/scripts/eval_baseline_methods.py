...
def historical_average_predict(df, period=12 * 24 * 7, test_ratio=0.2, null_val=0.0):
    """
    ...
    for i in range(n_train + period, n_sample, period):
        ...
        start = i - n_train
        y_predict.iloc[start : start + size, :] = y_predict.iloc[
...
def var_predict(df, n_forwards=(1, 3), n_lags=4, test_ratio=0.2):
    ...
    for input_ind in range(start, n_sample - n_lags):
        prediction = var_result.forecast(
            scaler.transform(df.values[input_ind : input_ind + n_lags]), max_n_forwards
        )
    ...
    for i, n_forward in enumerate(n_forwards):
        df_predict = pd.DataFrame(
            scaler.inverse_transform(result[i]),
            index=df_test.index,
...
def eval_static(traffic_reading_df):
    ...
    for horizon in horizons:
        ...
        rmse = masked_rmse_np(
            preds=y_predict.as_matrix(), labels=y_test.as_matrix(), null_val=0
        )
        mape = masked_mape_np(
            preds=y_predict.as_matrix(), labels=y_test.as_matrix(), null_val=0
        )
...
def eval_var(traffic_reading_df, n_lags=3):
    n_forwards = [1, 3, 6, 12]
    y_predicts, y_test = var_predict(
        traffic_reading_df, n_forwards=n_forwards, n_lags=n_lags, test_ratio=0.2
    )
    logger.info("VAR (lag=%d)" % n_lags)
    logger.info("Model\tHorizon\tRMSE\tMAPE\tMAE")
    for i, horizon in enumerate(n_forwards):
        rmse = masked_rmse_np(
            preds=y_predicts[i].as_matrix(), labels=y_test.as_matrix(), null_val=0
        )
        mape = masked_mape_np(
            preds=y_predicts[i].as_matrix(), labels=y_test.as_matrix(), null_val=0
        )
