...
class DCRNNModel(nn.Module, Seq2SeqAttrs):
    def __init__(self, adj_mx, logger, latent_dim=50, **model_kwargs):
        ...
        self.seq_len = int(model_kwargs.get("seq_len"))
        self.rnn_enc_in_dim = int(model_kwargs.get("rnn_units")) + int(
            model_kwargs.get("output_dim") * 2
        )
        self.rnn_enc_out_dim = int(model_kwargs.get("r_dim"))
        self.rnn_encoder = nn.GRU(
            self.rnn_enc_in_dim, self.rnn_enc_out_dim, self.num_rnn_layers_gru
        )
        self.rnn_dec_in_dim = (
            int(model_kwargs.get("rnn_units"))
            + int(model_kwargs.get("output_dim"))
            + int(model_kwargs.get("z_dim"))
        )
        self.rnn_dec_out_dim = int(model_kwargs.get("rnn_units_gru"))
        self.rnn_decoder = nn.GRU(
            self.rnn_dec_in_dim, self.rnn_dec_out_dim, self.num_rnn_layers_gru
        )
        self.fc = nn.Linear(
            int(model_kwargs.get("rnn_units_gru")), int(model_kwargs.get("output_dim"))
        )
        self.relu = nn.ReLU()
        self._logger = logger
    ...
    def data_to_z_params(self, start, x, y):
        ...
        prev_day_seq = torch.cat(
            [torch.unsqueeze(start, dim=0), (y[:-1]).clone().detach()], dim=0
        )
    ...
    def sample_z(self, mean, var, n=1):
        ...
        eps = torch.autograd.Variable(
            var.data.new(var.size(0), n, var.size(1)).normal_()
        ).to(device)
    ...
    def dcrnn_to_hidden(self, x):
        ...
        for t in range(self.embed_model.seq_len):
            output_hidden, encoder_hidden_state = self.embed_model(
    ...
    def decoder(self, start, outputs_hidden, zs, labels=None, batches_seen=None):
        ...
        decoder_hidden_state = torch.zeros(
            (self.num_rnn_layers_gru, batch_size, self.rnn_dec_out_dim), device=device
        )

        outputs = []

        for t in range(self.seq_len):
            decoder_output_interal, decoder_hidden_state = self.rnn_decoder(
                torch.cat(
                    [decoder_inputs[t : t + 1], zs[t : t + 1], decoder_output], dim=-1
                ),
    ...
    def forward(
        ...
        if test == False:
            ...
            self._logger.debug(
                "starting point complete, starting split source and target"
            )
            # first half for context, second for target
            x_c, y_c, start_c, x_t, y_t, start_t = self.split_context_target(
                inputs, labels, inputs0, self.context_percentage
            )
