...
class DCRNNSupervisor:
    def __init__(self, random_seed, iteration, max_itr, adj_mx, **kwargs):
        ...
        self.dcrnn_model = (
            dcrnn_model.cuda(device) if torch.cuda.is_available() else dcrnn_model
        )
    ...
    def save_model(self, epoch, z_total, outputs, saved_model, val_mae, mae, rmse):
        ...
        torch.save(
            config,
            "seed%d/itr%d/model_epo%d.tar" % (self.random_seed, self.iteration, epoch),
        )
        torch.save(
            z_total,
            "seed%d/itr%d/z_epo%d.tar" % (self.random_seed, self.iteration, epoch),
        )
        np.savez_compressed(
            "seed%d/itr%d/test_epo%d.npz" % (self.random_seed, self.iteration, epoch),
            **outputs
        )
        ...
        return "seed%d/itr%d/model_epo%d.tar" % (
            self.random_seed,
            self.iteration,
            epoch,
        )

    def load_model(self, random_seed, iteration, poch_num):
        if poch_num:
            self.random_seed = random_seed
            self.iteration = iteration
            self._epoch_num = poch_num
        assert os.path.exists(
            "./tools/seed%d/itr%d/z_epo%d.tar"
            % (self.random_seed, self.iteration, self._epoch_num)
        ), ("Z at epoch %d not found" % self._epoch_num)
        ...
        self._setup_graph()
        assert os.path.exists(
            "./tools/seed%d/itr%d/model_epo%d.tar"
            % (self.random_seed, self.iteration, self._epoch_num)
    ...
    def _setup_graph(self):
        with torch.no_grad():
            ...
            for _, (x, y, x0) in enumerate(val_iterator):
                ...
                output, _ = self.dcrnn_model(
                    x,
                    y,
                    x0,
                    test=True,
                    z_mean_all=self.z_mean_all,
                    z_var_temp_all=self.z_var_temp_all,
                )
                break
    ...
    def evaluate(
        self, dataset="val", batches_seen=0, z_mean_all=None, z_var_temp_all=None
    ):
        """
        ...
        with torch.no_grad():
            ...
            for _, (x, y, x0) in enumerate(val_iterator):
                ...
                output, _ = self.dcrnn_model(
                    x, y, x0, batches_seen, True, self.z_mean_all, self.z_var_temp_all
                )
            ...
            y_preds_scaled = np.exp(y_preds) - 1.0
            y_truths_scaled = np.exp(y_truths) - 1.0

            mae_metric, rmse_metric = self._test_loss(y_truths_scaled, y_preds_scaled)
            self._writer.add_scalar(
                "{} loss".format(dataset), mae_metric, batches_seen
            )  # check
        ...
        for epoch_num in range(self._epoch_num, epochs):
            ...
            for _, (x, y, x0) in enumerate(train_iterator):
                ...
                (
                    ...
                    z_var_temp_all_sub,
                    z_mean_context_sub,
                    z_var_temp_context_sub,
                ) = self.dcrnn_model(x, y, x0, batches_seen)
                ...
                mae_loss, kld_loss = self._compute_loss(
                    y_t,
                    output,
                    z_mean_all_sub,
                    z_var_temp_all_sub,
            ...
            if (epoch_num % log_every) == log_every - 1:
                message = (
                    "Epoch [{}/{}] ({}) train_mae: {:.4f}, train_kld: {:.4f}, val_mae: {:.4f}, lr: {:.6f}, "
                    "{:.1f}s".format(
                        epoch_num,
                        epochs,
                        batches_seen,
            ...
            if (epoch_num % test_every_n_epochs) == test_every_n_epochs - 1:
                test_mae_loss, test_rmse_loss, test_outputs = self.evaluate(
                    dataset="test",
                    batches_seen=batches_seen,
                    z_mean_all=self.z_mean_all,
                    z_var_temp_all=self.z_var_temp_all,
                ...
                message = (
                    "Epoch [{}/{}] ({}) test_mae: {:.4f}, test_rmse: {:.4f}, lr: {:.6f}, "
                    "{:.1f}s".format(
                        epoch_num,
                        epochs,
                        batches_seen,
            ...
            if val_loss < min_val_loss:
                ...
                saved_z_total = torch.stack(
                    [self.z_mean_all, self.z_var_temp_all], dim=0
                )
                ...
                if wait == patience:
                    ...
                    self._logger.info(
                        ...
                        "saving to {}".format(
                            saved_val_mae, saved_mae, saved_rmse, model_file_name
                        )
                    )
    ...
    def _compute_loss(
        ...
        y_true = torch.exp(y_true) - 1.0
        y_predicted = torch.exp(y_predicted) - 1.0

        return mae_loss(y_predicted, y_true), kld_gaussian_loss(
            z_mean_all, z_var_temp_all, z_mean_context, z_var_temp_context
        )
